...
/* como sería para sacar el key y el value pari de director: nombre*/
    let movieDirectors = moviesArray.map((elements) => elements.director);
...
function getAllUniqueDirectors(moviesArray) {
    let uniqueMovieDirectors = [...new Set(getAllDirectors(movies))];
...
/* como sería para hacer lo mismo teniendo el nombre del director y el género como variable? */

function howManyMovies(moviesArray) {
    let moviesSpielberg = moviesArray.filter( (elements)  => elements.director === "Steven Spielberg"|| elements.genre.includes("Drama"));
...
function dramaMoviesScore(moviesArray) {
    let dramaMovies = moviesArray.filter( (elements) => elements.genre.includes("Drama"));
    let dramaScore = dramaMovies.reduce((sum, element) => {
        return sum + element.score}, 0);
    let dramaAverageScore = +(dramaScore/dramaMovies.length).toFixed(2);
